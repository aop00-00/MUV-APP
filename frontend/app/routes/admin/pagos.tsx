// admin/pagos.tsx — Mi Estudio > Métodos de Cobro (Supabase)
...
export async function loader({ request }: Route.LoaderArgs) {
    ...
    for (const row of (data ?? [])) {
        connected[(row as any).provider] = (row as any).is_connected;
    }
...
export async function action({ request }: Route.ActionArgs) {
    ...
    if (intent === "disconnect") {
        await supabaseAdmin.from("payment_gateways")
...
function GatewayCard({ gateway, isConnected }: { gateway: GatewayDef; isConnected: boolean }) {
    ...
    return (
        <div className={`rounded-2xl border p-6 space-y-5 transition-all duration-200 ${optimisticConnected ? "border-green-400/40 bg-green-500/5" : gateway.color}`}>
            {/* Header */}
            <div className="flex items-start justify-between gap-4">
                ...
                {optimisticConnected ? (
                    ...
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/10 text-white/50 text-xs font-semibold shrink-0 border border-white/[0.08]">
                        <Circle className="w-3.5 h-3.5" /> Sin conectar
                    </span>
                )}
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
                {gateway.features.map((f) => (
                    <span key={f} className="px-2 py-0.5 rounded-full text-[11px] font-medium border"
                        style={{ color: gateway.accentColor, borderColor: gateway.accentColor + "33", background: gateway.accentColor + "0D" }}>
                        {f}
                    </span>
                ))}
            </div>

            {/* API Key input / Connected state */}
            {!optimisticConnected ? (
                <fetcher.Form method="post" className="space-y-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center border border-white/[0.08] rounded-xl overflow-hidden focus-within:border-gray-400 transition-colors bg-white/5">
                            <input name="api_key" type={showKey ? "text" : "password"} placeholder={gateway.keyPlaceholder}
                                className="flex-1 pl-3 pr-2 py-2.5 text-sm focus:outline-none bg-transparent font-mono text-white" required />
                            <button type="button" onClick={() => setShowKey(v => !v)} className="px-3 text-white/40 hover:text-white/70 transition-colors">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <fetcher.Form method="post" className="inline">
                        ...
                        <input type="hidden" name="provider" value={gateway.id} />
                        <button type="submit" className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-red-400 border border-red-400/30 hover:bg-red-500/10 transition-all active:scale-95">
        ...
        <div className="space-y-6 max-w-3xl">
            <div>
                ...
                <p className="text-white/50 text-sm mt-0.5">
                    Conecta tu pasarela de pago para que tus alumnos puedan pagar membresías, clases y paquetes directamente en tu estudio.
                </p>
            </div>
            ...
            <div className="space-y-4">
                {GATEWAYS.map((gateway) => (
                    <GatewayCard key={gateway.id} gateway={gateway} isConnected={connected[gateway.id] || false} />
                ))}
            </div>
        </div>
    );
