// admin/events.tsx — Workshops, talleres & unique one-time events (Supabase)
...
export async function loader({ request }: Route.LoaderArgs) {
    ...
    const { profile, gymId } = await requirePlanAccess(request, "/admin/events");
    const { getGymEvents } = await import("~/services/event.server");

    const rawEvents = await getGymEvents(gymId);
...
// ─── Component ───────────────────────────────────────────────────
export default function Eventos({ loaderData }: Route.ComponentProps) {
    const { events } = loaderData;
    const fetcher = useFetcher();
    const [showModal, setShowModal] = useState(false);
    const [form, setForm] = useState({ name: "", date: "", time: "10:00", capacity: 15, price: 0, location: "", description: "" });

    // Close modal after successful creation
    useEffect(() => {
        if (fetcher.state === "idle" && fetcher.data?.success && fetcher.data?.intent === "create") {
            setShowModal(false);
            setForm({ name: "", date: "", time: "10:00", capacity: 15, price: 0, location: "", description: "" });
        }
        ...
        <div className="space-y-6">
            ...
            {events.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {events.map(e => {
                        const pct = Math.round((e.current_enrolled / e.max_capacity) * 100);
                            ...
                            <div key={e.id} className="bg-white/[0.03] backdrop-blur-2xl rounded-2xl border border-white/[0.08] shadow-2xl p-5 space-y-4">
                                <div className="flex items-start justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm text-white/50">{e.location}</p>
                                    </div>
                                ...
                                <div className="grid grid-cols-3 gap-3 text-center">
                                    ...
                                    <div><p className="text-xs text-white/40 mb-0.5">Hora</p><p className="text-sm font-semibold text-white/90">{e.time}</p></div>
                                ...
                                <div>
                                    <div className="flex items-center justify-between text-xs text-white/50 mb-1">
                                        <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {e.current_enrolled}/{e.max_capacity} inscritos</span>
                ...
                <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white/5 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-6 space-y-4">
                            {[["Nombre del evento", "name", "Workshop Pilates Prenatal"], ["Ubicación", "location", "Sala A / Jardín exterior"]].map(([l, k, p]) => (
                                <div key={k}>
                        ...
                        <div className="p-6 border-t border-white/5 flex gap-3">
                            ...
                            <button onClick={save} disabled={!form.name || !form.date} className="flex-1 bg-amber-400 hover:bg-amber-500 disabled:bg-white/5/20 disabled:text-white/40 text-black font-bold px-4 py-2.5 rounded-xl text-sm">Guardar evento</button>
                        </div>
                    </div>
                </div>
            )}
